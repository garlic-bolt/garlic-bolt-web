...
public class RequestAgentTool {
	...
	/**
	 * 根据request uri获取block code
	 * @param uri
	 * @return
	 */
	public static String getBlockByUri(String uri){
		String[] param = uri.split("/");

		if(!Pattern.matches("\\d{4,6}",param[2]))
			throw new RuntimeException("url中找不到block code");
	...
	public static boolean isAndroidDevice(HttpServletRequest request) {
		String userAgent = request.getHeader("User-Agent");
		boolean isMobile = false;
		final String[] ios_sys = { "Android", "Linux" };
		for (int i = 0; !isMobile && userAgent != null && !userAgent.trim().equals("") && i < ios_sys.length; i++) {
			if (userAgent.contains(ios_sys[i])) {
				isMobile = true;
				break;
	...
	public static boolean isWeChat(HttpServletRequest request) {
		String userAgent = request.getHeader("User-Agent");
		return userAgent == null || userAgent.toLowerCase().indexOf("micromessenger") == -1 ? false : true;
	}
	...
	public static boolean isAlipay(HttpServletRequest request) {
		String userAgent = request.getHeader("User-Agent");
		return userAgent == null || userAgent.toLowerCase().indexOf("alipayclient") == -1 ? false : true;
	}
